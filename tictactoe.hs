...
-- Perform function on specified location on the board.
performMove :: Position -> (a->a) -> [[a]] -> [[a]]
performMove (y, x) f = changePieceHere y (\ycoord -> changePieceHere x f ycoord)
...
changePieceHere :: Int -> (a->a) -> [a] -> [a]
changePieceHere x a as = let (before, after) = splitAt x as
  in before ++ changePiece a after

-- Helper function for changePieceHere - modifies the head of a list
-- This helps us place a piece at the spot chosen by the user or computer
changePiece :: (a->a) -> [a] -> [a]
changePiece _ [] = []
changePiece f (a:as) = f a : as
...
-- Helper method for calculating computer's next move (hard)
getHardComputerMove :: Matrix -> Matrix
getHardComputerMove board = doMove (computerHardNextMove board) compNum board

-- Helper method for calculating computer's next move (easy)
